defmodule Computer do
  ...
  defp compute_sum(state, position) do
    value = Enum.at(state, Enum.at(state, position + 1)) + Enum.at(state, Enum.at(state, position + 2))
  ...
  defp replace(state, position, new_value) do
    Enum.with_index(state) |> Enum.map(fn {value, index} -> if index == position, do: new_value, else: value end)
  end
end
